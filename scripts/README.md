...
Installs and configures a complete email server stack for YektaYar (Postfix, Dovecot, RoundCube, PostfixAdmin).
...
**What it does:**
- Installs and configures Postfix (SMTP server)
- Installs and configures Dovecot (IMAP/POP3 server)
- Installs and configures OpenDKIM (email authentication)
- Downloads and installs PostfixAdmin (web-based email administration)
- Downloads and installs RoundCube (webmail interface)
- Creates mail database in PostgreSQL
- Configures SSL/TLS using existing Let's Encrypt certificates
- Sets up virtual mailboxes and domains
- Creates default mailbox: info@yektayar.ir (catch-all)
- Generates DKIM keys for email authentication
- Configures SPF and DMARC ready
- Sets up Apache virtual hosts for webmail and admin

**Prerequisites:**
- PostgreSQL must be installed (run `setup-postgresql.sh` first)
- Let's Encrypt SSL certificates must exist at `/etc/letsencrypt/live/yektayar.ir/`
- DNS access to add MX, A, TXT records

**Environment Variables:**
- `DOMAIN` - Email domain (default: `yektayar.ir`)
- `HOSTNAME` - Mail server hostname (default: `mail.yektayar.ir`)
- `ADMIN_EMAIL` - Default admin email (default: `info@yektayar.ir`)
- `WEBMAIL_SUBDOMAIN` - Webmail subdomain (default: `webmail.yektayar.ir`)
- `POSTFIXADMIN_SUBDOMAIN` - PostfixAdmin subdomain (default: `postfixadmin.yektayar.ir`)
- `DB_NAME` - Mail database name (default: `yektayar_mail`)
- `DB_USER` - Mail database user (default: `mail_admin`)
- `DB_PASSWORD` - Mail database password (auto-generated if not provided)

**Access Points:**
- Webmail: https://webmail.yektayar.ir
- PostfixAdmin: https://postfixadmin.yektayar.ir
- Default login: info@yektayar.ir / yektayar2024 (CHANGE THIS!)

**Post-Installation:**
1. Run `./scripts/setup-email-dns-records.sh` to see required DNS records
2. Add DNS records to your domain
3. Wait 15-30 minutes for DNS propagation
4. Run `./scripts/verify-email-setup.sh` to verify configuration
5. Complete PostfixAdmin setup at https://postfixadmin.yektayar.ir/setup.php
6. Change default password immediately

**See also:** [docs/EMAIL-SERVER-SETUP.md](../docs/EMAIL-SERVER-SETUP.md) for complete documentation

#### `verify-email-setup.sh`

Verifies email server configuration, DNS records, and service status.
...
**What it checks:**
- DNS records (MX, A, SPF, DKIM, DMARC, PTR)
- Service status (Postfix, Dovecot, OpenDKIM, Apache)
- Port accessibility (25, 587, 465, 993, 143, 995, 110)
- SSL certificate validity and expiration
- Database configuration
- Web access to webmail and PostfixAdmin
- DKIM key validation

**No root required** for most checks, but sudo recommended for full verification.

#### `setup-email-dns-records.sh`

Automatically generated by `setup-email-server.sh` - displays required DNS records for email server.
...
- A records for mail server, webmail, and PostfixAdmin
- SPF record for sender authentication
- DKIM record for email signing (includes your generated public key)
- DMARC record for email policy
- PTR record recommendation

**Note:** This script is informational only - you must manually add these records to your DNS provider.
