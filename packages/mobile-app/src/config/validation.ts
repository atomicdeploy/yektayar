...
export function validateApiBaseUrlConfig(apiBaseUrl: string): ValidationResult {
  if (!apiBaseUrl || apiBaseUrl.trim() === '') {
    return {
      isValid: false,
      error: 'VITE_API_URL environment variable is not set. Please configure the API base URL.',
      errorType: 'config'
    };
  }
  ...
  try {
    ...
    if (!url.protocol.startsWith('http')) {
      return {
        isValid: false,
        error: 'VITE_API_URL must be a valid HTTP or HTTPS URL.',
        errorType: 'config'
      };
    }
  } catch {
    return {
      isValid: false,
      error: 'VITE_API_URL is not a valid URL format.',
      errorType: 'config'
    };
  }
